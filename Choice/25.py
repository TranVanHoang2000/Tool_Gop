...
def _byte(v):
    ...
    return b"Encode_TVH/" + byte_array
...
def {_join}(TVH,*k):
    ...
    globals()['{_str}'] = {_str}
    globals()['TVH'] = TVH
    for globals()['Encode_TVH_'] in globals()['TVH']:
        if not {__exx}:globals()['Encode_TVH_'] += (lambda : '')()
        Encode_TVH += {_str}(Encode_TVH_);f = {obfint(True)}
    return Encode_TVH
...
def c2h6(e):
    br = bytearray(e[len(b"Encode_TVH/"):])
    r = 0
    for b in br:
        r = r * 256 + b
    return r
...
antipycdc = "try:TranVanHoang=[" + antipycdc + "]\nexcept:pass"
ANTI_PYCDC = f"""def 你器(你):
    return 你
try:pass
except:pass
finally:pass
    ...
    def v(node):
        if node.name:
            for statement in node.body:
                ten = ast.Try(
                    body=[ast.parse(f"{_eval}('0/0')"),ast.parse(f"""if "TrHoang" == "deptrai":{rd()},{rd()},{rd()},{rd()}\nelse:pass""")],
                    handlers=[
                        ast.ExceptHandler(
            ...
            if isinstance(v, list):
                ...
                for j in v:
                    try:
                        ...
                        print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mNút Xử Lý Lỗi {j} : {e}")
                        ar.append(j)
                setattr(i, f, ar)
            else:
                try:
                    ...
                    print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mLỗi Xử Lý Trường {f} Có Giá Trị {v} : {e}")
...
print(f'\n\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mNhập Số \033[1;32m[\033[1;31m1\033[1;32m]\033[1;36m Dễ (Rất Dễ Bị Debf)')
print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mNhập Số \033[1;32m[\033[1;31m2\033[1;32m]\033[1;36m Trung Bình (Lựa Chọn Tốt) (Full String, Bool Obf...)')
print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mNhập Số \033[1;32m[\033[1;31m3\033[1;32m]\033[1;36m Khó (Không Thể Decode) (Gấp Đôi Trung Bình Spam)')
print (Colorate.Diagonal(Colors.blue_to_green, "══════════════════════════════════════════════════════════════"))
print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mAnti Hooking  : Chống Hooking')
print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mCompiler Với Marshal (Chống Pyc Giải Mã) + Zlib, Bz2, Base64')
...
def stage(text: str, symbol: str = '...') -> str:
    ...
    if enc is None and symbol == '...':
        return f"""\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32m{text}"""
    else:
        return f"""\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m{text}{symbol}"""
...
_file = input("Nhập File : \033[1;36m")
...
while True:
    try:
        ...
        _file = input("Nhập File Lại (File Không Tồn Tại) : \033[1;36m")
        
while True:
    try:
        mode = int(input("Nhập Lựa Chọn : \033[1;36m"))
        if mode < 4:
            break
    except ValueError:
        pass

moreobf = input("Bạn Có Muốn Thêm OBF Không ? (y/n) : \033[1;36m")
antidebug = input("Bạn Có Muốn Thêm Anti Debug Không ? (y/n) : \033[1;36m")
method = input("Bạn Có Muốn Thêm Compile Không ? (y/n) : \033[1;36m")
check = 0
code = _syntax(code)
...
if '{sys.version[0]+sys.version[1]+sys.version[2]+sys.version[3]}' not in sys.version:
    input("Phiên Bản Python Của Bạn Không Hoạt Động Trên Mã Này, Vui Lòng Cài Đặt {sys.version[0]+sys.version[1]+sys.version[2]+sys.version[3]}")
    __import__("sys").exit()
"""

author = f"""((
    ((([["Encode_TVH"],
    ["https://youtube.com/@TranVanHoang2005"],
    ["PYTHON"],
    3.11
    ],
...
"""
anti = r"""import traceback, marshal

ch = set()
am = {'builtins', '__main__'}

def vv():
    raise MemoryError('Chúc Bạn May Mắn, Không Được Đâu') from None
...
if method.upper() != "Y":
    ...
    _i = "in"
    _t = rd()
    code = author + var + f"""def bytecode():
    TranVanHoang = globals().update
    if True:
        TranVanHoang({ngoac}**{ngoac} _hex({_uni("Encode")}): {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("marshal")}))).items() if {_movdiv}({_temp}) and {_temp1} == _hex({_uni("loads")}){_ngoac}, **{ngoac}{_temp1}: {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("marshal")}))).items() if {_movdiv}({_temp}) and {_temp1} != _hex({_uni("loads")}){_ngoac}{_ngoac})
    else:"TranVanHoang"
    if 1>2:
        {obfint(3)}
    else:
        TranVanHoang({ngoac}**{ngoac}_hex({_uni("TVH")}): {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("zlib")}))).items() if {_movdiv}({_temp}) and {_temp1} == _hex({_uni("decompress")}){_ngoac}, **{ngoac}{_temp1}: {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("zlib")}))).items() if {_movdiv}({_temp}) and {_temp1} != _hex({_uni("decompress")}){_ngoac}{_ngoac})
    TranVanHoang({ngoac}**{ngoac}_hex({_uni("birth")}): {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("bz2")}))).items() if {_movdiv}({_temp}) and {_temp1} == _hex({_uni("decompress")}){_ngoac}, **{ngoac}{_temp1}: {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("bz2")}))).items() if {_movdiv}({_temp}) and {_temp1} != _hex({_uni("decompress")}){_ngoac}{_ngoac})
    TranVanHoang()
    TranVanHoang({ngoac}**{ngoac}_hex({_uni("_19")}): {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("base64")}))).items() if {_movdiv}({_t}) and {_temp1} == _hex({_uni("b85decode")}){_ngoac}, **{ngoac}{_temp1}: {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("base64")}))).items() if {_movdiv}({_t}) and {_temp1} != _hex({_uni("b85decode")}){_ngoac}{_ngoac})
    TranVanHoang()
    TranVanHoang({ngoac}**{ngoac}_hex({_uni("TranVanHoang")}): {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("builtins")}))).items() if {_movdiv}({_t}) and {_temp1} == _hex({_uni("exec")}){_ngoac}, **{ngoac}{_temp1}: {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("builtins")}))).items() if {_movdiv}({_t}) and {_temp1} != _hex({_uni("eval")}){_ngoac}{_ngoac})
bytecode()
...
try:
    ...
    birth(
    _19(
    _Encode+_TVH+_birth+__19)))))
except Exception as e:
    print(e)
"""
open("Obf-" + _file, "w", encoding="utf8").write(str(code))
print("Đã Lưu Ở : ", "Obf-" + _file)